...
export default function Layout() {
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col">
      <header className="fixed top-0 left-0 w-full z-40 h-[79px] border-b border-gray-300 bg-white flex items-center px-4 sm:px-6 md:px-20 max-w-[1440px] mx-auto">
        <Link to="/">
          <img
            src={logo}
            alt="RentYard Logo"
        ...
        <div className="ml-auto">{renderHeaderButton()}</div>
      </header>

      {/* Main content with padding top and bottom equal to header/footer height, and flex-grow to fill available space */}
      <main className="flex-grow pt-[79px] pb-[96px] w-full max-w-[1440px] mx-auto px-4 sm:px-6 md:px-20">
        <Outlet />
      </main>

      {/* Footer fixed at bottom with height 96px */}
      <footer
        className="fixed bottom-0 left-0 w-full z-40 flex flex-col bg-white shadow-[0_-2px_12px_rgba(39,43,53,0.1)]"
        style={{ height: "96px" }}
      >
        {currentStep > 0 && (
          <div className="w-full h-1 flex items-center gap-2.5">
            {Array.from({ length: totalSteps }).map((_, index) => (
              <ProgressSegment key={index} isActive={index < currentStep} />
            ))}
          </div>
        )}
        <div className="h-[72px] max-w-[1440px] w-full mx-auto flex items-center justify-between px-4 sm:px-6 md:px-20">
          {footerActions}
        </div>
      </footer>
    </div>
